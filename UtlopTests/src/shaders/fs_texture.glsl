...
vec4 PointLight(){
	...
	return (texture(diffuse0, text_coords) * vec4(pointLightSTR.color,1.0f)) * (diffuse * inten + ambient) + texture(specular0, text_coords).r * specular * inten * 1.0f;
...
vec4 DirectionalLight(){
  ...
  vec3 lightCoords = fragPosLight.xyz / fragPosLight.w;
    ...
    float bias = max(0.0025f * (1.0f - dot(normal, lightDirection)), 0.005f);
...
vec4 SpotLight()
{
  ...
  vec3 lightCoords = fragPosLight.xyz / fragPosLight.w;
	...
	return (texture(diffuse0, text_coords) * (diffuse * (1.0f - shadow) * inten + ambient) + texture(specular0, text_coords).r * specular * (1.0f - shadow) * inten) * vec4(spotLightSTR.color,1.0f);
}
